...
C     ------------------------  ----------------------------------------
C     CALL DXSETUN(LUN)          Set the logical unit number, LUN, for
...
C                               immediately.
C     CALL DXSETF(MFLAG)         Set a flag to control the printing of
...
C     next DLSODE call for that problem.   In addition, if DXSETUN and/or
C     DXSETF was called for non-default handling of error messages, then
...
C  XERRWD, DXSETUN, DXSETF, DIXSAV, DIUMACH   handle the printing of all
C           error messages and warnings.  XERRWD is machine-dependent.
C Note: DVNORM, DUMACH, DIXSAV, and DIUMACH are function routines.
...
C (The routines DXSETUN and DXSETF are designed to conform to the
...
C 19820315  Upgraded DMDI in ODRV package: operates on M + M-transpose.
...
C 19830503  Added routine DCNTNZU; added NZL and NZU to /LSS001/;
C           changed DADJLR call logic; added optional outputs NZL & NZU;
...
C  DJGROUP   is called by DPREP to compute groups of Jacobian column
C           indices for use when MITER = 2.
C  DADJLR    adjusts the length of required sparse matrix work space.
C           It is called by DPREP.
C  DCNTNZU   is called by DPREP and counts the nonzero elements in the
...
C           driver routine which calls Subroutines DMD, DMDI, DMDM,
C           DMDP, DMDU, and SRO.  See Ref. 2 for details.  (The ODRV
...
C  DUMACH   computes the unit roundoff in a machine-independent manner.
C  XERRWD, DXSETUN, DXSETF, DIXSAV, and DIUMACH  handle the printing of all
C           error messages and warnings.  XERRWD is machine-dependent.
C Note:  DVNORM, DUMACH, DIXSAV, and DIUMACH are function routines.
C All the others are subroutines.
C
...
C-----------------------------------------------------------------------
 ...
 628  MSG='DLSODES- RWORK length insufficient (for Subroutine DPREP).  '
      ...
      GO TO 700
 629  MSG='DLSODES- RWORK length insufficient (for Subroutine DJGROUP).'
...
C  XERRWD, DXSETUN, DXSETF, DIXSAV, and DIUMACH  handle the printing of all
C           error messages and warnings.  XERRWD is machine-dependent.
C Note:  DMNORM, DFNORM, DBNORM, DUMACH, DIXSAV, and DIUMACH are
...
C  XERRWD, DXSETUN, DXSETF, DIXSAV, and DIUMACH  handle the printing of all
C           error messages and warnings.  XERRWD is machine-dependent.
C Note:  DVNORM, DDOT, DNRM2, DUMACH, DIXSAV, and DIUMACH are function
...
C  XERRWD, DXSETUN, DXSETF, DIXSAV, and DIUMACH  handle the printing of all
C           error messages and warnings.  XERRWD is machine-dependent.
C Note:  DVNORM, DDOT, DUMACH, DIXSAV, and DIUMACH are function routines.
C All the others are subroutines.
C
C-----------------------------------------------------------------------
...
C           Upgraded DMDI in ODRV package: operates on M + M-transpose.
C           Numerous revisions in use of work arrays;
C           use wordlength ratio LENRAT; added IPISP & LRAT to Common;
C           added optional outputs IPIAN/IPJAN;
C           Added routine DCNTNZU; added NZL and NZU to /LSS001/;
C           changed DADJLR call logic; added optional outputs NZL & NZU;
...
C  DCNTNZU   is called by DPREPI and counts the nonzero elements in the
C           strict upper triangle of P + P-transpose.
C  DJGROUP   is called by DPREPI to compute groups of Jacobian column
C           indices for use when MITER = 2.
C  DINTDY   computes an interpolated value of the y vector at t = TOUT.
C  DSTODI   is the core integrator, which does one step of the
C           integration and the associated error control.
...
C-----------------------------------------------------------------------
 ...
 628  MSG='DLSODIS- RWORK length insufficient (for Subroutine DPREPI). '
      ...
      CALL XERRWD (MSG, 60, 28, 0, 2, LENRW, LRW, 0, 0.0D0, 0.0D0)
      GO TO 700
 629  MSG='DLSODIS- RWORK length insufficient (for Subroutine DJGROUP).'
      CALL XERRWD (MSG, 60, 29, 0, 0, 0, 0, 0, 0.0D0, 0.0D0)
      MSG='        Length needed is .ge. LENRW (=I1), exceeds LRW (=I2)'
      CALL XERRWD (MSG, 60, 29, 0, 2, LENRW, LRW, 0, 0.0D0, 0.0D0)
      GO TO 700
