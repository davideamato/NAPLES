...
C         -2  insufficient storage for DJGROUP.
...
C Compute grouping of column indices (MITER = 2). ----------------------
      ...
      CALL DJGROUP (N, IWK(IPIAN), IWK(IPJAN), MAXG, NGP, IWK(IPIGP),
...
C
 230  IPPER = -3
      CALL DCNTNZU (N, IWK(IPIAN), IWK(IPJAN), NZSUT)
...
C----------------------- End of Subroutine DPREP -----------------------
      END
*DECK DJGROUP
      SUBROUTINE DJGROUP (N,IA,JA,MAXG,NGRP,IGP,JGP,INCL,JDONE,IER)
...
C Error return if not all columns were chosen (MAXG too small).---------
 ...
 80   IER = 1
      RETURN
C----------------------- End of Subroutine DJGROUP ----------------------
      END
*DECK DADJLR
      SUBROUTINE DADJLR (N, ISP, LDIF)
      INTEGER N, ISP, LDIF
      DIMENSION ISP(*)
C-----------------------------------------------------------------------
...
C NZLU = (size of L) + (size of U) = (IL(N+1)-IL(1)) + (IU(N+1)-IU(1)).
      ...
      RETURN
C----------------------- End of Subroutine DADJLR -----------------------
      END
*DECK DCNTNZU
      SUBROUTINE DCNTNZU (N, IA, JA, NZSUT)
...
c    the coding in subroutine DMDI, and in the initial comments in
c    subroutines odrv and DMD.
c
...
c              9n+k  insufficient storage in DMD
...
c***********************************************************************
c  DMD -- minimum degree algorithm (based on element model)
c***********************************************************************
c
c  description
c
c    DMD finds a minimum degree ordering of the rows and columns of a
...
c    ---------+-----------------------------+---------------------------
c    last(vi) - (not set until DMDP)         - -d =) vi head of d-list d
...
c ** error-  insufficient storage
 101  flag = 9*n + vi
     ...
     *     (vk,tail, v,l, last,next, mark)
c***********************************************************************
c  DMDM -- form element from uneliminated neighbors of vk
c***********************************************************************
...
c
     ...
     *     (k,ek,tail, v,l, head,last,next, mark)
c***********************************************************************
c  DMDP -- purge inactive elements and do mass elimination
c***********************************************************************
...
c
     ...
     *     (ek,dmin, v,l, head,last,next, mark)
c***********************************************************************
c  DMDU -- update degrees of uneliminated vertices in ek
c***********************************************************************
