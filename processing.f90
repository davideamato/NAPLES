...
subroutine RESIDUALS(npts,ref,test,sma,mu,RSw,dR_abs,dR_rel,dV_abs,dV_rel,dEn_rel,&
&RSw_eff,RSw_diff_max)
...
do i=1,npts
  ...
  dR_vec = ref(i,2:4)/sma - test(i,2:4)/sma
  dR_abs(i) = sqrt(dot_product(dR_vec,dR_vec))
  dR_rel(i) = dR_abs(i)/(r_ref/sma)
