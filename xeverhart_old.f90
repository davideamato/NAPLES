...
do
  ...
  tprev = t; xprev = x; vprev = v;
  ! RA15 STEP
  call XRA15_STEP(npq,ncl,FORCE,neq,x,v,t,h,f0,nf,ni,b,d,wc,wc0,uc,r,c)
  idiag(2) = idiag(2) + nf
  ns = ns + 1
  ...
  ! Check that the first step is valid.
  if (ns == 1) then
    call RA15_SCONTROL(neq,nes,nsf,nper,ncount,ss,(t-ta),tf,b,wc,h,&
    &hp,exc,istate)
    if (istate < 0) then
      ! Unable to find the initial stepsize. Is there a problem in FORCE?
      call CLEAN()
      return
    end if
  ...
  if (.not.(nsf)) then
    ! If the first step must be repeated, reset. Note that we use the b-values
    ! from the first tried (and failed) step in the initialization of the second
    ! tried step.
    x = xin; v = vin; t = tain; h = hp
    ns = 0
    cycle
        ...
        if (gcur*gprev <= eps_root) then
          ...
          if (abs(gprev) <= eps_root) then
            ...
            if (abs(dg2prev) <= eps_root .or. abs(dg2cur) <= eps_root) then
              ...
              idiag(5) = 1
              call BISECTION(eqs,neq,ns,ni,FORCE,tstar,tprev,xprev,vprev,gprev,&
              &dg2prev,hprev,f0prev,bprev,eprev,t,x,v,gcur,dg2cur,nfn,nin,bisec,&
              &troot,groot,xroot,vroot,d,wc,wc0,uc,r,c)
              if (bisec /= 1) then
            ...
            if (bisec /= 2) then
              ! NEWTON'S METHOD.
              dgroot = DSUND(eqs,neq,troot,xroot)
              call NEWTON(npq,ncl,neq,eqs,FORCE,tstar,troot,groot,dgroot,xroot,vroot,&
              &rconv,nin,nfn,tprev,xprev,vprev,hprev,f0prev,bprev,eprev,ns,ni,d,wc,wc0,uc,r,c)
              idiag(3) = idiag(3) + nfn
              idiag(6) = idiag(6) + nin
            end if
...
!    idiag(2) = idiag(2) + 1
  ...
  ! STEP SIZE CONTROL
  call RA15_SCONTROL(neq,nes,nsf,nper,ncount,ss,(t-ta),tf,b,wc,h,hp,exc,istate)
  if (exc) exit
  bprev = b; eprev = e
  call BPREDICT(neq,hp,h,ns,b,e)
  hprev = h
  h = hp
...
subroutine XRA15_STEP(npq,ncl,FORCE,neq,x,v,t,h,f0,nf,ni,b,d,wc,wc0,uc,r,c)
...
subroutine RA15_SCONTROL(neq,nes,nsf,nper,ncount,ss,tm,tf,b,wc,h,hp,exc,istate)
...
! INPUTS/OUTPUTS
logical,intent(inout)   ::  nsf,nper
real(dp),intent(inout)  ::  h
integer,intent(inout)   ::  ncount

! LOCALS AND PARAMETERS
integer,parameter  :: max_ncount = 10
real(dp),parameter :: sr = 1.4_dp, pw = one/9._dp
real(dp)  ::  hv,hval
...
if (nes) then
  ! If the step size is fixed, just set nsf = .true. if this is the first
  ! sequence.
  if (.not.(nsf)) nsf = .true.
  hp = h
  ...
  if (nsf) then
    ! It's not the first sequence. Limit the growth ratio between subsequent
    ! steps to 1.4.
    if (hp/h > sr) hp = h*sr
  
  else
    ! It's the first sequence. If the predicted step is smaller than the initial,
    ! decrease the size of the initial step and restart the integration.
    ! If this happens max_count times, return control to the caller.
    if (hp/h > one) then
      if (hp/h > sr) hp = h*sr
      nsf = .true.
...
&conv,niter,neval,s0,x0,v0,h0,f0,b0,e0,ns,ni,d,wc,wc0,uc,r,c)
...
! INPUTS
logical,intent(in)   ::  npq,ncl
integer,intent(in)   ::  neq,eqs,ns,ni
...
integer,intent(out)  :: niter,neval
...
do
  ...
  &,f0,nf,ni,b,d,wc,wc0,uc,r,c)
  neval = neval + nf
  
  groot  = TIMETOUT(eqs,neq,tstar,sroot,xroot)
  dgroot = DSUND(eqs,neq,sroot,xroot)
...
subroutine BISECTION(eqs,neq,ns,ni,FORCE,tstar,sprev,xprev,vprev,gprev,dg2prev,&
&hprev,f0prev,bprev,eprev,scur,xcur,vcur,gcur,dg2cur,neval,niter,cflag,s0_guess,&
&g0_guess,x0_guess,v0_guess,d,wc,wc0,uc,r,c)
! Initial guess for Newton's method when dg2prev*dg2cur < 0.

! INPUTS
integer,intent(in)  :: eqs,neq,ns,ni
...
integer,intent(out)  ::  neval,niter
